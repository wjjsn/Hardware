...
private:
	...
	static void wait_for_write_end()
	{
		...
		uint8_t read_status_register = READ_STATE_REGISTER;
		spi_device::select();
...
public:
	static uint32_t read_id()
	{
		...
		uint8_t buf[4] = {READ_ID, DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE};
	...
	{
		write_enable();
		uint8_t send_buf[4] = {ERASE_SECTOR,
	...
	{
		write_enable();
		uint8_t chip_erase = ERASE_BILK;
		spi_device::transmit(&chip_erase, 1);
		wait_for_write_end();
	}
	static void page_write(uint8_t *pbuffer, uint32_t write_addr, uint16_t num_byte_to_write)
	{
		write_enable();
		uint8_t send_buf[4] = {WRITE_DATA,
									static_cast<uint8_t>(write_addr >> 16),
									static_cast<uint8_t>(write_addr >> 8),
									static_cast<uint8_t>(write_addr >> 0)};
	...
	static void block_write(uint8_t *pbuffer, uint32_t write_addr, uint16_t num_byte_to_write)
	{
	...
	static void buffer_read(uint8_t *pbuffer, uint32_t read_addr, uint16_t num_byte_to_read)
	{
		// 命令+地址 和 数据 需要在同一个 CS 窗口内，因此手动控 CS
		uint8_t send_buf[4] = {READ_DATA,
									static_cast<uint8_t>(read_addr >> 16),
									static_cast<uint8_t>(read_addr >> 8),
									static_cast<uint8_t>(read_addr >> 0)};
		spi_device::select();
		spi_device::transmit_without_ctl_select(send_buf, 4);
